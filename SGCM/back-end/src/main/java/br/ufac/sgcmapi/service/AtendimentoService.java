...
public class AtendimentoService implements ICrudService<Atendimento> {
    ...
    ProfissionalService servicoProfissional, AtendimentoOLAPRepository repoOLAP) {
        this.repo = repo;
    ...
    private void atualizaIndicadorAusencia(){
        ...
        List<EStatusAtendimento> status = new 
        ArrayList<EStatusAtendimento>();
        status.add(EStatusAtendimento.AGENDADO);
        status.add(EStatusAtendimento.CONFIRMADO);

        Date data = new Date(System.currentTimeMillis());

        List<Atendimento> atendimentos = repo
        .findByStatusInAndDataGreaterThanEqual(status, data);

        atendimentos.forEach(item -> {

            AtendimentoOLAP registro = repoOLAP.findById(item.getId())
            .orElse(null);
            ...
            if(registro != null) {
                ...
                try {
                    ...
                    instacias = DataSource.read("paciente_ausente.arff");
                    instacias.setClassIndex(instacias.numAttributes() - 1);

                    double[] valores = new double[instacias.numAttributes()];
                    valores[0] = instacias.attribute(0).indexOfValue(registro.getAtendimentoHora());
                    valores[1] = instacias.attribute(1).indexOfValue(registro.getProfissionalEspecialidade());
                    valores[2] = instacias.attribute(2).indexOfValue(registro.getUnidadeNome());
                    valores[3] = instacias.attribute(3).indexOfValue(registro.getConvenioNome());
                    valores[4] = instacias.attribute(4).indexOfValue(registro.getPacienteSexo());
                    valores[5] = instacias.attribute(5).indexOfValue(registro.getPacienteFaixaEtaria());
                    valores[6] = instacias.attribute(6).indexOfValue(registro.getPacienteNovo());
                    valores[7] = instacias.attribute(7).indexOfValue(registro.getPacientePossuiAusencias());
                    valores[8] = Utils.missingValue();
                                        
                    Instance instacia = new DenseInstance(1.0, valores);
                    instacias.add(instacia);

                    double resultado = 0;

                    File modelo = ResourceUtils.getFile("classpath:paciente_ausente.model");
                    InputStream isModelo = new FileInputStream(modelo);
                    Classifier classificador = (Classifier)
                    SerializationHelper.read(isModelo);

                    resultado = classificador.classifyInstance(instacias.lastInstance());

                    String novaClasse = instacias.classAttribute().value((int) resultado);
